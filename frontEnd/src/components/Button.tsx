...
const Button = ({
	...
	const variantStyles = {
		...
		"outline-1":
			"border border-grey-0 hover:bg-grey-1 hover:border-grey-1 hover:text-white",
		"outline-2":
			"border border-grey-4 hover:bg-grey-1 hover:border-grey-1 hover:text-white",
	...
	return (
		<button
			{...rest}
			type={type}
			className={`${fullWidth ? "w-full" : "w-fit"} p-2 ${
				fullRounded ? "rounded-full" : "rounded"
			} ${
				variantStyles[variant]
			} text-b1 font-medium transition-colors duration-300  md:text-b0`}
		>
			{children}
		</button>
	);
