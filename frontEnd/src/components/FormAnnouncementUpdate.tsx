...
export const FormUpdateAnnouncement = ({ fetchData, car }: iPropsUpdateAnouncement) => {
  ...
  const updatedAnnouncement = async (data: iAnnouncements) => {
    ...
    await api.put(`/announcements/${car.id}`, formatedData, { headers })
      .then(() => {
        fetchData()
        closeModal();
      })
  ...
  const deleteAnnouncement = async () => {
    const headers = { Authorization: `Bearer ${token}` };
    await api.delete(`/announcements/${car.id}`, { headers })
      .then(() => {
        fetchData()
        closeModal();
      })
  ...
  return (
    <form onSubmit={handleSubmit(updatedAnnouncement)}>
      ...
      <div className="flex justify-end w-full gap-3 pr-6 my-6">
        <Button type="button" variant="grey-6" onClick={() => deleteAnnouncement()}>
          Excluir Anuncio
        </Button>
